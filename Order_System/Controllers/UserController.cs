...
public class UserController : ControllerBase
{
    ...
    public IActionResult GetUserListByUserId(long userID)
    {
        try
        {
            ...
            List<UserResponseModel> lst = _adoDotNetService.Query<UserResponseModel>(
                query,
                parameters.ToArray()
            );
    ...
    public IActionResult Register([FromBody] RegisterRequestModel requestModel)
    {
        try
        {
            ...
            if (
                string.IsNullOrEmpty(requestModel.PhoneNo)
                || !(
                    requestModel.PhoneNo.Length <= 13
                    && Regex.IsMatch(requestModel.PhoneNo, @"^\+\d{1,12}$")
                )
            )
            {
                return BadRequest(
                    "PhoneNumber cannot be empty and must be 10 digits with an optional leading +."
                );
            }
            ...
            if (
                ...
                || !(
                    ...
                    && Regex.IsMatch(requestModel.Password, @"[!@#$%^&*()_+=\[{\]};:<>|./?,-]")
                )
            ) //ThetHtar@8
            {
                return BadRequest(
                    "Password cannot be empty and must contain at least 8 characters including uppercase letters, lowercase letters, a number, and special characters."
                );
            }
